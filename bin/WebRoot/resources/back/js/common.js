 ...
 * 定义全局变量
 */
var img_done = "<img src='" + STATIC_URL + "/back/images/ico-done.gif' border='0' />";
var img_delete = "<img src='" + STATIC_URL + "/back/images/ico-delete.gif' border='0' />";
var img_warning = "<img src='" + STATIC_URL + "/back/images/ico-warning.gif' border='0' />";
var img_loading_small = "<img src='" + STATIC_URL + "/back/images/loading-small.gif' border='0' />";
	...
	function processSessionTimeout(xhr) {
    	...
    	if (sessionstatus == "timeout") { 
	    	bootbox.alert("登陆超时,请重新登陆", function(){
	    		window.location.replace( BASE_URL + '/back/adminLogin/login '+ (redirectUri ? ('?urlContinue=' + redirectUri) : '')); 
	    		});
	    }
	    if (sessionstatus == "notPermission") { 
	    	bootbox.alert("您暂时没有该功能的访问权限！", function(){
	    		window.location.replace( BASE_URL + '/back/ '+ (redirectUri ? ('?urlContinue=' + redirectUri) : '')); 
	    		});
	    }
