...
function user_submit() {
	...
	if (!$("input[name=idcard]").val().length == 18 || !$("input[name=idcard]").val().length == 15) {
		notice('edit_notice', img_delete + ' 请正确填写身份证号码', true, 5000);
		return false;
	}
	...
	if (userid == '' || userid == 0) {
		...
		$("#edit_form").attr("action", BASE_URL + "/back/adminUser/edit");
