...
public class FrontFilter implements Filter {
    ...
    /**
     * 过滤前端需登录才能访问的url 若未登录，或登录过期，就跳转到登录页面
     * 
     * @author haojian
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
        ...
        //调试用，测试web端是否能传递cookie过来
        String bmSessionId = CookieUtil.getCookieValue(request, SessionManager.BM_SESSION_ID);
        LOGGER.debug("web端cookie中的bmSessionId=【{}】", new Object[]{bmSessionId} );
        ...
        //默认跳转到主页,用户地址未输入完全的时候，跳转到电商主页面
        if(servletPath.equals("")||servletPath.equals("/")||servletPath.equals("/front")||servletPath.equals("/front/")||servletPath.equals(null)){
            response.sendRedirect(Config.BASE_URL + "/front/index/index");
            return;
        }
        
        //3、记录用户行为操作日志
        LogUtil.logUserAction(request, LogConstant.LOG_TYPE_FRONT);
    ...
    /**
     * session过期后，返回resultTO的json对象到客户端（手机端或网页端） 客户端应解析此消息，并跳转到登录界面
     * 
     * @author haojian
     * @date 2016-1-12 下午3:43:54
     * @param request
     ...
     * @return void
     */
    private void processRedirect(HttpServletRequest request, HttpServletResponse response, String uri) throws IOException {
    	...
    	if (AjaxUtil.checkIsAjax(request)) {
    		...
    		response.sendRedirect(Config.BASE_URL + "/front/cas/login");
    	}
    }
