...
public class Test {
    ...
    @Before
    public void setUp() {
        pool = new JedisPool(new JedisPoolConfig(), "172.51.97.111");
    ...
    public List<GoodsCategory> getGoodsCategoryList() {
        List<GoodsCategory> list = goodsCategoryDao.selectObjectList(null);
        // 所有分类map，key：categoryId
        Map<Integer, GoodsCategory> map = new HashMap<Integer, GoodsCategory>();
        // 顶级分类列表
        List<GoodsCategory> topList = new ArrayList<GoodsCategory>();
        ...
        for (GoodsCategory c : list) {
            ...
            // 找到子集的父级对象
            GoodsCategory parent = map.get(c.getParentId());
            parent.getSubList().add(c);
        }
