...
//收藏商品js
function collect_goods(content_id,type){
    $.post(BASE_URL+'/front/cas/favorite/add', {content : content_id, type : type}, function(response){
        if (response.status == 0){
            alert(response.msg);
            $("#emfavoriteId").html(response.data);
        } else {
            alert(response.msg);
        }
    },'json');
...
function load_cart_information(){
    $.post(BASE_URL + '/front/cart/getFrontCart', function(result){
        ...
        if(result){
            ...
            if(result.data.count >0){
                ...
                for (var i = 0; i < carts.length; i++){
                    var goods = carts[i]['description'];
                    html+='<dl id="cart_item_'+carts[i]['cartId']+'"><dt class="goods-name"><a href="'+BASE_URL+'/front/goodsContent/detail?skuId=' + carts[i]['skuId'] +'" target="_blank">'+carts[i]['name'] +'</a></dt>';
                    html+='<dd class="goods-thumb"><a href="'+BASE_URL+'/front/goodsContent/detail?skuId='+ carts[i]['skuId'] +'" title="'+carts[i]['name']+'" target="_blank"><img src="'+IMG_URL+carts[i]['DefaultImage']+'"></a></dd>';
                    html+='<dd class="goods-sales"></dd>';
                    html+='<dd class="goods-price"><em>'+price_format(carts[i]['subPrice'])+'×'+carts[i]['quantity']+'</dd>';
                    html+='<dd class="handle"><a href="javascript:void(0);" onClick="drop_topcart_item('+carts[i]['cartId']+','+carts[i]['skuId']+');">删除</a></dd>';
                    html+="</dl>";
                }
                obj.find('.incart-goods').html(html);
                obj.find('.incart-goods-box').perfectScrollbar('destroy');
                obj.find('.incart-goods-box').perfectScrollbar();
                html = "共<i>"+result.data.count+"</i>种商品&nbsp;&nbsp;总计金额：<em>"+price_format(result.data.totalprice)+"</em>";
                obj.find('.total-price').html(html);
                if (obj.find('.addcart-goods-num').size()==0) {
                    obj.append('<div class="addcart-goods-num">0</div>');
                }
                obj.find('.addcart-goods-num').html(result.data.count);
                $('#rtoobar_cart_count').html(result.data.count).show();
            } else {
...
function drop_topcart_item(cartId,skuId){
    $.post(BASE_URL + '/front/cart/deleteGoodsFromCart', {cartId:cartId, skuId:skuId}, function(result){
        if(result.status == 0){
            var obj = $('.head-user-menu .my-cart');
            //删除成功
            if(result.data.count == 0){
                ...
                $('#cart_item_' + cartId).remove();        //移除
                html="共<i>"+result.data.count+"</i>种商品&nbsp;&nbsp;总计金额：<em>&yen;"+result.data.totalprice+"</em>";
                obj.find('.total-price').html(html);
                obj.find('.addcart-goods-num').html(result.data.count);
                obj.find('.incart-goods-box').perfectScrollbar('destroy');
                obj.find('.incart-goods-box').perfectScrollbar();
                $('div[ncType="rtoolbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;" + result.data.totalprice + "</em>");
                $("#rtoobar_cart_count").html(result.data.count);
            }
        }else{
            alert(result.msg);
        }
...
/* 加入购物车 */
function addcart(contentId, quantity, callbackfunc) {
