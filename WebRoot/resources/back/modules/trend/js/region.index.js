$(document).ready(function() {
	...
	$('body').delegate('.operate-delete', 'click', function() {
		...
		$.ajax({
			type : 'post',
			url : BASE_URL + '/back/trend/region/delete',
			data : param,
	        dataType:'json',
	        timeout:10000,
...
function region_extend(region_id) {
	var tr_template = '<tr id="tr_region_{#region_id#}" class="{#region-pid#}">' + 
	    	...
	    	'<td>{#sort_order#}</td>' + 
	    	'<td>' + 
	    	'<a href="' + BASE_URL + '/back/trend/region/add?pid={#region_id#}" data-toggle="ajaxModal" class="operate-edit">添加子地区</a> | ' + 
	    	'<a href="' + BASE_URL + '/back/trend/region/update?regionId={#region_id#}" data-toggle="ajaxModal" class="operate-edit">编辑</a> | ' + 
	    	'<a href="javascript:;" class="operate-delete" regionId="{#region_id#}"  parentId="{#parentId#}">删除</a>' + 
	    	'</td>';
	...
	$.ajax({
    	type:'post',
        url:BASE_URL + '/back/trend/region/getChilds',
