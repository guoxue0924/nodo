    ...
    <div class="ncc-main">
        ...
        <form action="${BASE_URL}/front/bts/order/check" method="POST" id="form_buy" name="form_buy">
            ...
            <table class="ncc-table-style" nc_type="table_cart">
                ...
                <tr>
                    <th class="w50"><label>
                        <input type="checkbox" checked="checked" value="1" id="selectAll">
                        全选</label></th>
                    <th></th>
                    <th>商品</th>
                ...
                <tr id="cart_item_${c.cartId}" nc_group="${c.cartId}" class="shop-list ">
                    ...
                    <td class="w60">
                        <a href="${BASE_URL}/front/goodsContent/detail?skuId=${c.skuId}" target="_blank" class="ncc-goods-thumb">
                            <c:if test="${c.DefaultImage != null && c.DefaultImage != ''}">
                                <img src="${IMG_URL}${c.DefaultImage}" alt="${c.name}">
                            </c:if>
                            <c:if test="${c.DefaultImage == null || c.DefaultImage == ''}">
                                <img src="${BASE_URL}/resources/front/images/goods/nopic.jpg" alt="${c.name}">
                            </c:if>
                        </a>
                    </td>
                    <td class="tl">
                    ...
                    <td class="w120">
                        <em id="item${c.cartId}_subtotal" subPrice="${c.subPrice}" nc_type="eachGoodsTotal"><fmt:formatNumber value="${c.subPrice}" pattern="￥#,#00.00#"/></em>
                    </td>
                    <td class="w80">
                ...
                <tfoot>
                <tr>
<%--                     <td colspan="20"><div class="ncc-all-account">商品总价（不含运费）<em id="cartTotal" nc_type="eachStoreTotal"><fmt:formatNumber value="${price}" pattern="￥#,#00.00#"/></em>元</div></td> --%>
                    <td colspan="20"><div class="ncc-all-account">商品总价（不含运费） ￥<em id="allTotal" subPrice="${c.subPrice}"  nc_type="eachStoreTotal"></em>元</div></td>
                </tr>
                </tfoot>
            </table>
        </form>
